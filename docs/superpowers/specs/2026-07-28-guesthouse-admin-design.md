...
## 부록: 머지 이후 모델 변경 (2026-07-28)

이 설계를 구현하는 사이, 같은 조상에서 갈라진 원격 브랜치에 19커밋이 쌓였다.
공휴일 API 연동, 폼 UI 개편, 손글씨 입력 개선이 들어 있었고 세 파일(`index.html`,
`draw.html`, `calc.js`)이 겹쳤다. 원격을 기준으로 삼고 리팩터를 재적용하기로 하면서
아래를 바꿨다.

### 좌표는 그대로다

원격의 `POS`(index)와 `CELLS`(draw) 9개가 변경되지 않았다. 본문의 실측 표와
`rectToPoint()` 변환, 회귀 방지 테스트는 전부 그대로 유효하다.
동·호수를 화면에서 두 칸으로 나눴지만 `readValues()`가 `'N동 M호'`로 합쳐 내보내므로
**서식에 찍히는 값도 동일하다.**
...
| 유형 | 상태 | 사유 |
|---|---|---|
| `toggle` | **삭제** | 공휴일 수동 토글이 특일정보 API 자동 판정으로 대체됨 |
| `money` | **삭제** | 금액을 직접 수정할 수 없게 바뀜 |
| `unit` | 추가 | 동(4자리)·호(3자리) 두 칸을 받아 한 문자열로 출력 |
| `readout` | 추가 | 사람이 못 고치는 자동 계산 표시칸 (사용일수·사용금액) |

`system` 항목에서 `holiday`가 빠져 6개가 되었다:
`checkIn`, `checkOut`, `period`, `nights`, `amount`, `people`.
...
"stay": {
  "limitDates": true,
  "maxAheadMonths": 1,  // 오늘부터 한 달 뒤까지만 선택 가능
  "minNights": 1,
  "maxNights": 2        // 퇴실일은 입실일 +1 ~ +2일
},
"holiday": {
  "enabled": true,
  "eveOfHoliday": true  // 공휴일 '전날 밤'도 주말 요금
}
```

`pricing.maxNights`와 `maxNightsText`는 삭제했다. 원격은 경고를 띄우는 대신 달력에서
아예 고르지 못하게 막으므로 경고 문구가 닿을 일이 없다. **운영 규정상 최대 2박이
맞다는 것을 확인**받아 본문의 "5박 이상 경고"는 폐기한다.

특일정보 API 서비스키는 `index.html`에 그대로 둔다. 설정으로 옮기면 관리자가 바꿀 수
있게 되지만 노출 지점이 하나 늘어나므로, 요청에 따라 현행 유지한다.

### 배치 규칙의 적용 범위 축소

원격 `index.html`이 2열 배치를 없애고 `.field-list` 안에 한 줄씩 쌓는 구조가 되어,
`packRows()`는 **`draw.html` 전용**이 되었다. `index.html`은 `formFields()` 순서대로
그대로 쌓는다.

`fields` 순서도 서식 문서 순서에 맞춰 재정렬했다(은행입금일이 마지막). 그 결과
`draw.html`의 손글씨 칸 순서가 바뀐다 — 기존에는 신청일·은행입금일이 맨 위에
짝지어 있었으나, 이제 은행입금일이 맨 아래로 간다. 서식의 실제 칸 위치(y=454로 가장
아래)와 일치하므로 개선으로 본다.

### `calcAmount` 시그니처

요금표와 공휴일 판정이 둘 다 필요해 이름 있는 옵션으로 받는다.

```js
calcAmount(values, { pricing, isHoliday })
```

위치 인자로 `(values, pricing, isHoliday)`처럼 나란히 두지 않는다. 이번 충돌이 정확히
그 형태였다 — 양쪽이 두 번째 자리를 각각 다른 용도로 썼고, 텍스트로는 충돌이 나지
않을 수도 있는 종류의 어긋남이었다.
