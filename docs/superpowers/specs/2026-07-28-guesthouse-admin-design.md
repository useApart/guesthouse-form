...
최대 오차 0.5px — 반올림 차이일 뿐 같은 좌표다. 따라서 **사각형을 유일한 진실**로 삼고
...
있으므로 출력에 변화가 없다. 테스트는 중심점을 정확히, `maxWidth`는 현재 값 ±3px 이내로 검증한다.
...
값과 일치하는지 검증한다 (중심점은 반올림 후 정확히 일치, `maxWidth`는 ±3px 이내).
