...
| `manage.html` | 관리사무소용 승인 화면 (로그인·목록·확정/취소) | 신규 |
| `index.html` | 달력 + 신청 흐름 추가 | 수정 |
...
신청 완료 화면에 "신청서 이미지 받기(선택)" 버튼으로 남긴다.

관리사무소가 새 방식에 익숙해지기 전까지 문자를 병행할 수 있고, 예약 시스템에 문제가
생겨도 기존 경로가 살아 있다. 안정된 뒤에 정리한다.
