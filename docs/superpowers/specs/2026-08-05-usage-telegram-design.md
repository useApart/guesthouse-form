...
텔레그램에 뜨는 숫자가 틀리는 것뿐이고, 데이터가 새지는 않는다. 방어 장치를 넣을
값어치가 없다.

다만 `bump_usage`는 예약 시스템과 같은 무료 Supabase 프로젝트에 붙어 있고, 인증
없이 무제한으로 두드릴 수 있는 쓰기 엔드포인트다. 작정하고 반복 호출하면 무료
티어 한도를 소진시킬 수 있고, 그러면 같은 프로젝트를 쓰는 예약 신청·달력 조회도
함께 죽는다. 블라스트 반경이 "틀린 숫자"에 그치지 않는다는 뜻이다.

지금 방어 장치를 넣지는 않는다. 실제로 남용이 보이면 그때 IP당 제한을 넣는다.
지금 구조를 바꾸지 않는다.
...
**별도 파일로 두는 이유.** 브라우저 ES 모듈에는 트리 셰이킹이 없어서
`import { buildRequest } from './reservations.js'` 한 줄도 결국 `reservations.js`
전체(약 13KB)를 내려받아 파싱하게 만든다 — "필요한 것만 재사용"은 안 되고,
실제로는 파일 전체를 끌어오는 셈이다. 그래도 감수한다: 13KB는 `form.jpg`
96KB 옆에서 반올림 오차이고, `reservations.js`는 최상위 부수효과가 없어
기능적 위험도 없다. 반대로 헤더 조립(`buildRequest`)을 이 파일에도 따로 베껴
두면 한쪽만 고치고 다른 쪽을 놓치는 사고가 난다 — 그 위험이 13KB보다 크다.
